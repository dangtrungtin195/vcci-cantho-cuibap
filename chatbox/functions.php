...
######## CHECK MD5 ##############
function check_chatbox_key($key, $userid,$usergroup)
{
	global $config;
	$genkey = md5($userid.$config['chatboxkey'].$usergroup);
...
{
	$userg = strip_tags($username);
	$userg = "<span onclick=\"click_chat('$userg');\" style=\"cursor: pointer;\"></span>";
	return $userg;
}
...
function build_chat($shout)
{
	...
	if ($config['use_me'] && $checkmecm == $command['me'].' ')
			...
			*/
			...
			$shout['message'] = BBCode($shout['message']);
			### parser format
			$shout['message'] = formattext($shout['message'], $shout['bold'], $shout['italic'], $shout['underline'], $shout['font'], $shout['color']);

			if ($isme)
			{
				return "isme > $shout[userid] > $shout[username] > $shout[dateline] > *".click_chat($shout['username'])."<a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[userid]' target='_blank'><b>$shout[username]</b></a> $shout[message]*";
			
			}
			else
			{
				return "chat > $shout[userid] > $shout[username] > $shout[dateline] > ".click_chat($shout['username'])."<a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[userid]' target='_blank'><b>$shout[username]</b></a>: $shout[message]";
			}
}
...
{
	global $config,$phrase;
	return "prune1 > $shout[userid] > $shout[username] > $shout[dateline] > <span class='smallfont'>Nick: ".click_chat($shout['username'])."<b>$shout[username]</b> $phrase[prune] !</span>";
}

function build_prune2($shout)
{
	global $config,$phrase;
	$text = "prune2 > $shout[userid] > $shout[username] > $shout[dateline] > <span class='smallfont'>#".click_chat($shout['username'])."<b>$shout[username]</b> $phrase[pruneusernotice] <a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[pruneuserid]' target='_blank'>$shout[pruneusername]</a>";
...
{
	global $config,$phrase;
	$text = "ban > $shout[userid] > $shout[username] > $shout[dateline] > <span class='smallfont'>#".click_chat($shout['username'])."<b>$shout[username]</b> ";
	if (!empty($shout['banusername']))
	{
		$text .= "$phrase[banned_name] <a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[banuserid]' target='_blank'>$shout[banusername]</a>";
	}
	else
	{
		$text .= "$phrase[banned] <a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[banuserid]' target='_blank'>$shout[banuserid]</a>";
	}
	if ($shout['reason']) $text .= ". <i>$phrase[reason]: $shout[reason].</i>";
	$text .= '#</span>';
	return $text;
...
{
	global $config,$phrase;
	$text = "unban > $shout[userid] > $shout[username] > $shout[dateline] > <span class='smallfont'>#".click_chat($shout['username'])."<b>$shout[username]</b> ";
	if (!empty($shout['unbanusername']))
	{
		$text .= "$phrase[unbanned_name] <a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[unbanuserid]' target='_blank'>$shout[unbanusername]</a>";
	}
	else
	{
		$text .= "$phrase[unbanned] <a href='http://{$config['cbforumlink']}/home.php?mod=space&uid=$shout[unbanuserid]' target='_blank'>$shout[unbanuserid]</a>";
	}
	if ($shout['reason']) $text .= ". <i>$phrase[reason]: $shout[reason].</i>";
	$text .= '#</span>';
	return $text;
